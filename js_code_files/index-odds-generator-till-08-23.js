...
const { get_kg, get_all_horses_kids, get_kids_and_upload } = require("./horses-kids");
...
const cutoff_date = "2021-08-24T00:00:00.000Z";
...
const struct_details_of_hid_zed = (data) => {
...
const get_details_of_hid = async (hid) => {
  try {
    ...
    // return struct_details_of_hid_zed(data);
    return get_details_of_hid_db(hid);
  } catch (err) {
    await delay(100);
...
const get_details_complete_str = async (hid) => {
  ...
  let ob = {
    ...
    hex_code,
    horse_type,
    rating_blood: { cf, d, med, side, rated_type, tc },
    rank: doc.rank,
  };
  return ob;
};
